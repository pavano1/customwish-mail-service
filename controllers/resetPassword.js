...
export const resetPassword = async (data, callback) => {
  try {
    ...
    await transporter.sendMail({
      ...
      to: data?.email,
      subject: `CustomWish Reset Password-Email:${data?.email}`,
      html: `
        ...
        <div style="margin: 0; margin-top: 30px; padding: 50px 30px 115px; background: #ffffff; border-radius: 30px; text-align: center;">
          ...
          <div style="width: 100%; max-width: 489px; margin: 0 auto; padding-top: 60px;">
            ...
            <p style="margin: 0; margin-top: 17px; font-weight: 500; letter-spacing: 0.56px; color: #2a2462;">
              Kindly find the OTP to reset the password. OTP is
            ...
            <p style="margin: 0; margin-top: 60px; font-size: 40px; font-weight: 600; letter-spacing: 25px; background-image: url('https://thecustomwish.com/public/uploads/images/gradient.png'); background-size: cover; color: #fff; padding: 10px; display: inline-block; border-radius: 8px;">
            ${data?.otp}
            </p>
          </div>
        </div>
